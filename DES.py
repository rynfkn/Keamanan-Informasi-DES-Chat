import base64

# Initial Permutation Table
IP = [
    58, 50, 42, 34, 26, 18, 10, 2,
    60, 52, 44, 36, 28, 20, 12, 4,
    62, 54, 46, 38, 30, 22, 14, 6,
    64, 56, 48, 40, 32, 24, 16, 8,
    57, 49, 41, 33, 25, 17, 9,  1,
    59, 51, 43, 35, 27, 19, 11, 3,
    61, 53, 45, 37, 29, 21, 13, 5,
    63, 55, 47, 39, 31, 23, 15, 7,
]

# Inverse Initial Permutation Table
IP_INV = [
    40, 8, 48, 16, 56, 24, 64, 32,
    39, 7, 47, 15, 55, 23, 63, 31,
    38, 6, 46, 14, 54, 22, 62, 30,
    37, 5, 45, 13, 53, 21, 61, 29,
    36, 4, 44, 12, 52, 20, 60, 28,
    35, 3, 43, 11, 51, 19, 59, 27,
    34, 2, 42, 10, 50, 18, 58, 26,
    33, 1, 41, 9,  49, 17, 57, 25,
]
# Expansion Permutation Table
E = [
    32, 1,  2,  3,  4,  5,
    4,  5,  6,  7,  8,  9,
    8,  9,  10, 11, 12, 13,
    12, 13, 14, 15, 16, 17,
    16, 17, 18, 19, 20, 21,
    20, 21, 22, 23, 24, 25,
    24, 25, 26, 27, 28, 29,
    28, 29, 30, 31, 32, 1,
]
# Permutation Box Table
P = [
    16, 7,  20, 21,
    29, 12, 28, 17,
    1,  15, 23, 26,
    5,  18, 31, 10,
    2,  8,  24, 14,
    32, 27, 3,  9,
    19, 13, 30, 6,
    22, 11, 4,  25,
]

# Subtitution Boxes (S-Boxes)
SBOXES = [
    [
        [14,4,13,1,2,15,11,8,3,10,6,12,5,9,0,7],
        [0,15,7,4,14,2,13,1,10,6,12,11,9,5,3,8],
        [4,1,14,8,13,6,2,11,15,12,9,7,3,10,5,0],
        [15,12,8,2,4,9,1,7,5,11,3,14,10,0,6,13]
     ],
    [
        [15,1,8,14,6,11,3,4,9,7,2,13,12,0,5,10],
        [3,13,4,7,15,2,8,14,12,0,1,10,6,9,11,5],
        [0,14,7,11,10,4,13,1,5,8,12,6,9,3,2,15],
        [13,8,10,1,3,15,4,2,11,6,7,12,0,5,14,9]],
    [
        [10,0,9,14,6,3,15,5,1,13,12,7,11,4,2,8],
        [13,7,0,9,3,4,6,10,2,8,5,14,12,11,15,1],
        [13,6,4,9,8,15,3,0,11,1,2,12,5,10,14,7],
        [1,10,13,0,6,9,8,7,4,15,14,3,11,5,2,12]],
    [
        [7,13,14,3,0,6,9,10,1,2,8,5,11,12,4,15],
        [13,8,11,5,6,15,0,3,4,7,2,12,1,10,14,9],
        [10,6,9,0,12,11,7,13,15,1,3,14,5,2,8,4],
        [3,15,0,6,10,1,13,8,9,4,5,11,12,7,2,14]],
    [
        [2,12,4,1,7,10,11,6,8,5,3,15,13,0,14,9],
        [14,11,2,12,4,7,13,1,5,0,15,10,3,9,8,6],
        [4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14],
        [11,8,12,7,1,14,2,13,6,15,0,9,10,4,5,3]],
    [
        [12,1,10,15,9,2,6,8,0,13,3,4,14,7,5,11],
         [10,15,4,2,7,12,9,5,6,1,13,14,0,11,3,8],
        [9,14,15,5,2,8,12,3,7,0,4,10,1,13,11,6],
        [4,3,2,12,9,5,15,10,11,14,1,7,6,0,8,13]],
    [
        [4,11,2,14,15,0,8,13,3,12,9,7,5,10,6,1],
         [13,0,11,7,4,9,1,10,14,3,5,12,2,15,8,6],
         [1,4,11,13,12,3,7,14,10,15,6,8,0,5,9,2],
        [6,11,13,8,1,4,10,7,9,5,0,15,14,2,3,12]],
    [
        [13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7],
        [1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2],
        [7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8],
        [2,1,14,7,4,10,8,13,15,12,9,0,3,5,6,11]],
]

# Permuted Choice Tables
PC1 = [
    57,49,41,33,25,17,9,1,58,50,42,34,26,18,
    10,2,59,51,43,35,27,19,11,3,60,52,44,36,
    63,55,47,39,31,23,15,7,62,54,46,38,30,22,
    14,6,61,53,45,37,29,21,13,5,28,20,12,4,
]

PC2 = [
    14,17,11,24,1,5,3,28,15,6,21,10,
    23,19,12,4,26,8,16,7,27,20,13,2,
    41,52,31,37,47,55,30,40,51,45,33,48,
    44,49,39,56,34,53,46,42,50,36,29,32,
]

ROTATIONS = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]

# Utils
def bytes_to_bits(data):
    bits = []
    for byte in data:
        for i in range(7, -1, -1):
            bits.append((byte >> i) & 1)
    return bits

def bits_to_bytes(bits):
    result = []
    for i in range(0, len(bits), 8):
        byte_val = 0
        for j in range(8):
            byte_val = (byte_val << 1) | bits[i + j]
        result.append(byte_val)
    return bytes(result)

def int_to_bits(value, length):
    bits = []
    for i in range(length - 1, -1, -1):
        bits.append((value >> i) & 1)
    return bits

def bits_to_int(bits):
    result = 0
    for bit in bits:
        result = (result << 1) | bit
    return result

def permute(bits, table):
    return [bits[pos - 1] for pos in table]

def xor_bits(bits1, bits2):
    return [b1 ^ b2 for b1, b2 in zip(bits1, bits2)]

def pkcs7_pad(data):
    pad_length = 8 - (len(data) % 8)
    if pad_length == 0:
        pad_length = 8

    padding = bytes([pad_length] * pad_length)
    return data + padding

def pkcs7_unpad(data):
    pad_length = data[-1]
    return data[:-pad_length]

def rotate_left(bits, n):
    return bits[n:] + bits[:n]

def generate_subkeys(key64):
    key56 = permute(key64, PC1)

    C = key56[:28]
    D = key56[28:]

    subkeys = []
    for round in range(16):
        rotation = ROTATIONS[round]
        
        C = rotate_left(C, rotation)
        D = rotate_left(D, rotation)

        CD = C + D

        subkey = permute(CD, PC2)
        subkeys.append(subkey)

    return subkeys

def sbox_subtituion(bits48):
    result = []
    for i in range(8):
        start = i * 6
        chunk = bits48[start:start + 6]

        row = (chunk[0] << 1) | chunk[5]
        col = (chunk[1] << 3) | (chunk[2] << 2) | (chunk[3] << 1) | chunk[4]

        sbox_value = SBOXES[i][row][col]

        result.extend(int_to_bits(sbox_value, 4))

    return result

def fiestel(right_bits, subkey):
    expanded = permute(right_bits, E)
    xored = xor_bits(expanded, subkey)
    subtituted = sbox_subtituion(xored)
    permuted = permute(subtituted, P)
    return permuted

def DES(input_bits, subkeys, decrypt=False):
    permuted = permute(input_bits, IP)

    L = permuted[:32]
    R = permuted[32:]

    for round in range(16):
        if decrypt:
            K = subkeys[15 - round]
        else:
            K = subkeys[round]
        
        L_old = L
        L = R

        f_output = fiestel(R, K)
        R = xor_bits(L_old, f_output)

    combined = R + L
    output = permute(combined, IP_INV)
    return output


def encrypt(plaintext_bytes, key_bytes):
    key_bits = bytes_to_bits(key_bytes)
    subkeys = generate_subkeys(key_bits)

    padded = pkcs7_pad(plaintext_bytes)

    chipertext = []
    for i in range(0, len(padded), 8):
        block = padded[i:i+8]
        block_bits = bytes_to_bits(block)

        encrypted_bits = DES(block_bits, subkeys, decrypt=False)
        encrypted_block = bits_to_bytes(encrypted_bits)

        chipertext.append(encrypted_block)

    return b''.join(chipertext)

def decrypt(ciphertext_bytes, key_bytes):
    key_bits = bytes_to_bits(key_bytes)
    subkeys = generate_subkeys(key_bits)
    
    plaintext = []
    for i in range(0, len(ciphertext_bytes), 8):
        block = ciphertext_bytes[i: i+8]
        block_bits = bytes_to_bits(block)

        decrypted_bits = DES(block_bits, subkeys, decrypt=True)
        decrypted_block = bits_to_bytes(decrypted_bits)
        plaintext.append(decrypted_block)

    padded = b''.join(plaintext)
    return pkcs7_unpad(padded)

def encrypt_text(text, key):
    text_encoded = text.encode('utf-8')
    encrypted = encrypt(text_encoded, key)
    return base64.b64encode(encrypted).decode('ascii')

def decrypt_text(token, key):
    ciphertext = base64.b64decode(token)
    decrypted_bytes = decrypt(ciphertext, key)
    return decrypted_bytes.decode('utf-8')

def main():
    print("=" * 25)
    print("  DES Encryption Program")
    print("=" * 25)
    print()

    text = input("Masukkan teks: ").strip()
    key = input("Masukkan kunci (8 karakter): ").encode("utf-8")

    if len(key) != 8:
        print("[!] Kunci harus tepat 8 byte (8 karakter ASCII).")
        return
    
    print("\nPilih mode operasi:")
    print("1. Enkripsi")
    print("2. Dekripsi")
    choice = input("Masukkan pilihan (1/2): ").strip()

    if choice == "1":
        encrypted_b64 = encrypt_text(text, key)
        print("\nHasil Enkripsi (Base64):")
        print(encrypted_b64)

    elif choice == "2":
        try:
            decrypted_text = decrypt_text(text, key)
            print("\nHasil Dekripsi:")
            print(decrypted_text)
        except Exception as e:
            print(f"[!] Terjadi kesalahan saat dekripsi: {e}")

    else:
        print("[!] Pilihan tidak valid. Gunakan 1 atau 2.")

    print("\n" + "=" * 25)

if __name__ == "__main__":
    main()